...
def main():
    ...
    df = all_tariffs[['year','hs','brief_description']].dropna(subset=['brief_description'])
    df['brief_description'] = df['brief_description'].astype(str)

    years = sorted(df['year'].unique())
    if len(years) < 2:
        raise RuntimeError("Need at least two years to map N → N+1.")

    # Descending pair iteration: (2022→2023), (2021→2022), … 
    for year_N, year_N1 in zip(years[::-1][1:], years[::-1][:-1]):
        print(f"\nProcessing {year_N} → {year_N1} …")

        df_N  = df[df['year'] == year_N ].copy()
        df_N1 = df[df['year'] == year_N1].copy()
        if df_N.empty or df_N1.empty:
            print(f"  Skipping {year_N} → {year_N1} (no data).")
            continue

        mapped_df = process_tariffs_parallel(
            df_N,    # queries: older year
            df_N1,   # candidates: next year
            tokenizer, model, device,
            alpha=0.7, beta=0.2, gamma=0.1, top_k=500
        )

        outfn = f"mapped_{year_N}_{year_N1}.csv"
        mapped_df.to_csv(outfn, index=False)
        print(f"  Saved mapping to {outfn}")

    print("\nAll backward-forward year-pairs processed.")
